...
public class CheckServiceFeasibilityResource {
    ...
    public Uni<CheckServiceAbilityResponse> checkServiceAbility(CheckServiceAbilityRequest request) {
        try {
            ...
            logger.error("An error occurred during process checkServiceAbility operation", exception);
    ...
    public Uni<CheckServiceFeasibilityResponse> checkServiceFeasibility(CheckServiceFeasibilityRequest request) {
        try {
            ...
            logger.error("An error occurred during process checkServiceFeasibility operation", exception);
            return Uni.createFrom().failure(() -> ExceptionUtil.buildTMFError(HttpStatus.SC_INTERNAL_SERVER_ERROR, exception.getMessage()));
        }
    }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path(value = "/checkQueryAncillaryServices")
    public Uni<QueryAncillaryServicesResponse> queryAncillaryServices(QueryAncillaryServicesRequest request) {
        try {
            if (request == null || request.isEmpty()) {
                return Uni.createFrom().failure(() -> ExceptionUtil.buildTMFError(HttpStatus.SC_BAD_REQUEST, ExceptionConstants.EMPTY_REQUEST));
            }
            return checkServiceFeasibilityService.queryAncillaryServices(request);
        } catch (Exception exception) {
            logger.error("An error occurred during process checkQueryAncillaryServices operation", exception);
            return Uni.createFrom().failure(() -> ExceptionUtil.buildTMFError(HttpStatus.SC_INTERNAL_SERVER_ERROR, exception.getMessage()));
        }
    }
